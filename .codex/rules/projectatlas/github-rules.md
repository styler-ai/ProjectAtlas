...
## Release workflow
- Update the Cargo workspace version in `Cargo.toml` to prepare a release from `dev`.
- Merge the release PR into `main` after CI is green; auto-release publishes the tag with generated notes.
- Release tags must match the Cargo version, for example `v0.3.0`.
- Ensure the GitHub Release notes include a **Highlights** section; edit the release if the auto-generated notes omit it.

## Local verification
- Run the Rust verification stack before pushing (pre-push hook enforces this): format, check, clippy, tests, rustdoc, map, and lint.

...
fn agent_purpose_and_health_resolution_gate_flow() -> Result<(), Box<dyn Error>> {
    ...
    for format in ["toon", "json"] {
        ...
        if health.status != health_check.status
    ...
    for (command, expected) in [("health", "resolve"), ("health-check", "--summary-only")] {
        let help = Command::cargo_bin("projectatlas")?
            ...
            .arg("--help")
            .output()?;
        if !help.status.success() || !String::from_utf8_lossy(&help.stdout).contains(expected) {
            return Err(io::Error::other(format!("{command} help omitted {expected}")).into());
        }
    }

...
/// Immutable policy whose digest binds the exact eager Linux runtime mappings.
#[cfg(all(target_os = "linux", target_arch = "x86_64"))]
const NATIVE_IMPORT_POLICY_FILE_NAME: &str = "native-import-policy.json";
...
impl PreparedParserRuntime {
    /// Eagerly initialize the system runtime before Linux containment.
    ...
    fn admit_language(
        ...
        let grammar = approve_grammar_selection(pack_root, requested.as_str())
...
fn parse_worker_invocation(
    ...
    } else if argument == SERVE_ARGUMENT {
...
/// Load the hash-bound exact eager Linux DSO set before installing containment.
#[cfg(all(target_os = "linux", target_arch = "x86_64"))]
fn load_runtime_mapping_policy(pack_root: &Path) -> Result<Vec<String>, WorkerStartupError> {
    let artifact_path = pack_root.join(ARTIFACT_MANIFEST_FILE_NAME);
    let artifact_bytes = read_bounded_pack_file(
        &artifact_path,
        "artifact manifest",
    ...
    if artifact.platform != PackPlatform::LinuxX86_64 {
        return Err(WorkerStartupError::InvalidRuntimeMappingPolicy {
            reason: "artifact target is not Linux x86-64",
        });
    }

    let policy_path = pack_root.join(NATIVE_IMPORT_POLICY_FILE_NAME);
    let policy_bytes = read_bounded_pack_file(
        &policy_path,
        "native-import policy",
        NATIVE_IMPORT_POLICY_MAX_BYTES,
    )?;
    if sha256_hex(&policy_bytes) != artifact.native_audit.policy_sha256.as_str() {
        return Err(WorkerStartupError::InvalidRuntimeMappingPolicy {
...
fn process_request(
    ...
    if let Err(code) = runtime.admit_language(pack_root, request.language()) {
...
fn serve_session(
    ...
    while let Some(request_bytes) = read_exact_frame(input, "request")? {
        ...
        if !process_request(pack_root, runtime, input, output, &request)? {
            break;
        }
    }
    Ok(())
...
fn serve_standard_streams(
    ...
    serve_session(pack_root, runtime, &mut input, &mut output, containment)
}
...
fn serve_worker(pack_root: &Path) -> Result<(), WorkerStartupError> {
    ...
    let expected_runtime_mappings = load_runtime_mapping_policy(pack_root)?;
    let mut runtime = PreparedParserRuntime::new(pack_root);
    let preconditions = observe_parser_worker_preconditions(pack_root, &expected_runtime_mappings)
        .map_err(|source| WorkerStartupError::LinuxContainment {
            stage: source.stage(),
            source,
        })?;
    let pack_root =
        enforce_parser_worker_containment(pack_root, preconditions).map_err(|source| {
...
mod tests {
    ...
    /// Accept only the two exact single-argument operations.
    #[test]
    fn invocation_is_closed_and_contains_no_grammar_argument()
    -> Result<(), Box<dyn std::error::Error>> {
        require(
            parse_worker_invocation([OsString::from(SERVE_ARGUMENT)].into_iter())?
                == WorkerInvocation::Serve,
    ...
    fn session_open_emits_exact_artifact_bound_ready() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut runtime = PreparedParserRuntime::new(pack.path());
        serve_session(
            pack.path(),
            &mut runtime,
            &mut input,
    ...
    fn forged_raw_source_emits_invalid_source_failure() -> Result<(), Box<dyn std::error::Error>> {
        ...
        require(
            !process_request(pack.path(), &mut runtime, &mut input, &mut output, &request)?,

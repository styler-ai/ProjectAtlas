...
18. After ProjectAtlas plugin/runtime updates, verify Codex global MCP registry drift with `codex mcp get projectatlas` or `codex mcp list` when `codex` is available. A stale global `projectatlas` entry pointing at an old runtime, wrong version, or invalid config is a bug; rerun the ProjectAtlas installer so it repairs the registry automatically. A global entry whose default DB belongs to another repository can still serve the current repository through process-scoped `atlas_set_project_path` or per-call `project_path`; prefer per-call `project_path` for shared or concurrent hosts. Use `PROJECTATLAS_SKIP_CODEX_MCP_REGISTRY_UPDATE=1` only for intentionally managed global registries.

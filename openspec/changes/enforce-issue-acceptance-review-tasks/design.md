...
Contract provenance is separate from mutable issue headings. `openspec/issue-map.json` owns one sorted, unique `legacy_closed_issues` list of mapped issue numbers. Every mapped issue outside that list is a new-contract issue, regardless of which headings a user later edits. Each listed issue is valid only while it remains mapped, CLOSED, contains exactly one visible legacy task heading with no new task fields, mirrors its exact local owner slice, and has no unchecked historical task; the checker fails closed otherwise. The list is populated from authenticated live state during the initial migration, so it contains only actual closed historical legacy issues. The initial pull-request boundary validates every derived and listed exception against that complete historical contract before accepting the provenance field, reusing the one fetched payload per mapped issue. A pull request may introduce the field when its accepted base predates it, but once an accepted base contains the field, candidate and base sets must match exactly. This per-issue authority handles mixed-owner change mappings without issue-number or timestamp heuristics.
...
Pushes to `main`, milestone completion, and release validation retain the complete global live-state check, including the listed legacy exceptions' closed-state, exact local-slice, and zero-unchecked-task proof. Ordinary issue events retain their existing affected-issue `--planned-issue` scope and apply that same legacy proof when the affected issue is grandfathered. This is not eventual consistency and does not permit stale owner progress: it separates candidate ownership from unrelated mutable state while preserving the existing global convergence boundary after merge.

...
3. **Use a handle-bound retirement guard.** Candidate selection requires one stable-path process whose parsed final argument is `mcp` and whose inspectable parent is the exact `codex.exe` process that launched it. Selection captures both processes' creation times, image paths, and complete argv, and associates the probed obsolete child version with a before/after SHA-256 image identity. Immediately before retirement the version and digest are reprobed. The retirement helper holds both handles while it revalidates both identities and the child's full argv and image digest, then terminates only the child handle.

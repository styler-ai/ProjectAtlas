...
- Keep the token-impact TUI focused on reconciled persisted impact data, including the observed and modeled file-read split plus exact broad-folder-walk and candidate-open source rows without duplicate expanded charts; keep comparison evidence in structured CLI/MCP output even when an explicit benchmark artifact is supplied; and use only real resolved SQLite relations for its bounded connected, clustered, depth-cued static wide-layout atlas.

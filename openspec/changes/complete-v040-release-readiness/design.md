...
- Keep the normal no-artifact token-impact TUI focused on reconciled persisted impact data, including the observed and modeled file-read split plus modeled directory-walk steps; retain comparison evidence only behind the explicit benchmark-artifact input; and use only real resolved SQLite relations for its bounded connected, clustered, depth-cued static wide-layout atlas.
...
The TUI consumes the same `TokenOverview` used by structured output, so all numeric fields remain derived rather than independently recalculated or fabricated. Its normal no-artifact view hides comparison content. When a caller explicitly supplies a benchmark artifact, one bounded separate panel renders the typed comparison without changing any live total. The detail strip preserves the report's file-read total and observed/modeled split, while its folder-walk count saturatingly sums only persisted modeled buckets whose baseline and denominator are both `directory_walk`; the source table remains the accounting reconciliation. Wide terminals additionally seed from bounded resolved non-containment relation-family reads, expand through two rounds of the existing indexed inbound and outbound adjacency API, retain one connected component under 48-node, 64-edge, and per-node degree ceilings, and fail closed when the optional graph read is unavailable. The renderer stores only those sampled visual nodes and edges, labels the result as a bounded live snapshot, and hides it at narrower widths. Unresolved or unavailable graph data is shown as empty or unavailable rather than replaced with demo content.

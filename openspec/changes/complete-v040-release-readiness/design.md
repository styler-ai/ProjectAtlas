...
- Keep the token-impact TUI focused on reconciled persisted impact data, including the observed and modeled file-read split plus modeled directory-walk steps, and use only real resolved SQLite relations for its bounded connected, clustered, depth-cued static wide-layout atlas.

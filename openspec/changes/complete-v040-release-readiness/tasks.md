...
- [x] 4.6 Keep the token-impact TUI limited to real persisted token, lookup, and likely file-read-call data; remove version, plain-control, and repeated-work comparisons; render a bounded non-interactive mini atlas from real resolved SQLite graph relations only in the wide layout; support dark, light, and terminal-background themes; and pass deterministic arithmetic, data-source, graph-bound, narrow-layout, and real-terminal visual review.

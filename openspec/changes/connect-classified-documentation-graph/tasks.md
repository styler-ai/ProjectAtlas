...
- [x] 1.1 Map issue #440 to this OpenSpec change and define the release scope, non-goals, ownership boundaries, and dependency on #430 and #448.
...
- [x] 1.5 Specify immutable-seed composition, private per-worktree database refresh, exact `project_path` isolation, dirty-source authority, ordinary-checkout behavior, and seamless v0.4.4 upgrade paths with #430.
...
- [ ] 5.1 Extend #430's portable allowlist and manifest identity with schema-17 classifications, headings, canonical document relations, provenance, completeness, and unresolved evidence while retaining every local/private-state exclusion.
- [ ] 5.2 Refuse sealing an incomplete classified generation and prove the content-addressed exact-tag seed remains immutable, read-only, privacy-safe, and unchanged by hydration.
- [ ] 5.3 Hydrate two private ignored worktree databases, rebind each exact root, refresh both sides of the seed/current diff, and prove main-only removal, branch-only addition, rename/delete/case change, and dirty saved bytes independently.
- [ ] 5.4 Prove simultaneous explicit `project_path` requests, captured request context, session-default changes, and ambiguous bare/common managers cannot mix classifications, relations, purposes, generations, or next calls between sibling worktrees.
- [ ] 5.5 Prove ordinary and linked-worktree v0.4.4 databases migrate in place without losing purposes, existing compatible local databases win, missing databases hydrate or build, and offline/no-Git fallback needs no manual database surgery.

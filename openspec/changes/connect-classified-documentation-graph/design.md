...
Add `Documents` to the existing extended graph family. A Markdown document is the canonical source entity and its validated indexed file/symbol target is the relation target. `documented_by` is only the inbound adapter label/view of the same stored fact. Self-links and duplicates collapse under canonical relation keys; cycles remain ordinary bounded graph cycles.
...
#430 adds the schema-17 classification table, heading symbols, document relations, and their completeness/provenance to the portable derived-state allowlist. The clean-main sealer includes them only after a complete #440 generation and excludes all local identities, telemetry, sessions, tasks, paths, caches, and writable sidecars as already specified.

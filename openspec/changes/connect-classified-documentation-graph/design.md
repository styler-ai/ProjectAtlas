...
#430 separately owns bounded structural worktree discovery, exact source-root selection, agent-readable root status, and explicit per-call `project_path` routing. Each checkout already owns one private ignored writable atlas. #440 adds semantic derived state inside that existing exact-root model; it must not add another database or let two worktrees share one graph.
...
### Compose with #430 exact worktree ownership

#440 owns the live schema-17 classification, heading, document, completeness, and unresolved facts in the selected checkout's existing atlas. Existing schema-16 atlases migrate locally; a checkout without an atlas uses ordinary local init and scan. No common-manager, sibling, release artifact, or second database carries graph authority.
...
Explicit per-call `project_path` captures the correct root/database/generation before traversal. #430 reports structural ambiguity at a common manager and requires an exact worktree instead of guessing; no classification, unresolved selector, relation, purpose, or next call may come from a sibling checkout.
...
1. Land #430's structural discovery, exact-root selection, and bounded status without changing database ownership.
...
5. Run ordinary-checkout and linked-worktree E2E: migrate v0.4.4 atlases, locally initialize a missing atlas, diverge documentation/source, refresh, and prove isolated bidirectional traversal plus structural status.
...
#440 depends on #430 for structural discovery, exact-root selection/status, explicit `project_path` routing, and the joint holistic worktree E2E. #430 depends on #440 only for the classified-document assertions exercised by that E2E. #448 owns RC-first release policy and prevents either issue from being treated as released before the candidate gates pass.

The ownership boundary is strict: #430 selects and reports one source checkout without opening its graph; #440 defines and refreshes classified documentation semantics inside that checkout's existing atlas. Neither issue may introduce a shared writable graph, merge branch databases, or duplicate the other's root/selection logic.

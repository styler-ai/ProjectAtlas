...
- Keep every checkout's classification/document facts in its own ignored writable database selected by a #430 registered `worktree` alias or legacy exact `project_path`; never merge sibling graph generations.
- Let explicit read-only federation accept registered worktree aliases while retaining root/worktree labels and never persisting a combined graph.
- Preserve a seamless v0.4.4 upgrade for ordinary checkouts and linked worktrees, including #430 safe hydration from a valid local main atlas and visible ordinary full-init fallback.
- Update the v0.4.5-rc1 shipped ProjectAtlas skill, public GitHub/Pages guidance, architecture documentation, and real multi-platform/worktree tests for the complete alias workflow.
...
- `worktree-classified-navigation`: Branch-local classified document refresh, structural worktree registration, short-selector routing, labelled read-only federation, exact-root isolation, and v0.4.4 upgrade/hydration compatibility.
...
- #430 structural worktree discovery, durable registration, short-selector routing, targeted init/hydration, aggregate token authority, shipped-skill guidance, and the joint holistic worktree E2E.

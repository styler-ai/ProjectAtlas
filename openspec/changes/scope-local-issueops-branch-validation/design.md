...
The hook retains global validation for any push targeting `refs/heads/main`, and uses the candidate route only for one valid non-main branch target whose local object is the current `HEAD` and whose worktree is clean. Multiple non-main targets, deletions, local-object mismatches, or dirty candidate state fail closed. CI pull requests remain PR-scoped, issue events remain planned-issue scoped, and release validation remains global. The real self-test remains an independent mandatory command in every existing owner.

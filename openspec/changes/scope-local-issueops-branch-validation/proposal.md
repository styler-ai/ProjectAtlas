...
- Route pre-push validation from exactly one pushed remote ref: one `refs/heads/main` update remains global, while one non-main `refs/heads/*` update whose non-zero local object is the checked-out `HEAD` uses candidate mode; zero, multiple, mixed, deleted, or mismatched updates fail closed and release checks remain global. Candidate mode also requires a clean worktree with no tracked `assume-unchanged` or `skip-worktree` index entries as an early mutation guard; the issue map, mapped task files, and linked architecture documents must each be tracked regular files in the candidate tree, and their UTF-8 contents are read directly from that candidate's Git blobs rather than the worktree. Every linked architecture document must be Markdown there; every post-base commit subject, including merge commits, carries exactly one same-owner `(#NNN)` reference with no unmatched `(#` fragment.
- Candidate commit, tree, blob, and accepted-base object reads disable local Git replacement refs so submitted object IDs remain authoritative.

...
- [The planning PR waits for body publication while body publication waits for the planning merge] -> Require exact-head independent Sol and hosted Codex review first, publish/read back the thirty bodies while the PR is still open, and keep the temporary body-to-main link gap fail-closed for readiness until the green PR merges and exact main is read back.
...
3. While that reviewed PR remains open and the authoritative/live graph remains at twenty-five children, primary Sol publishes and reads back the exact thirty bodies from the reviewed drafts. The temporary body-to-main architecture-link gap remains fail-closed for readiness.
...
6. Primary Sol applies and reads back the complete hosted milestone and twenty-nine-child native parent/blocker bootstrap from the accepted manifest. Any partial, raced, or mismatched hosted state remains fail-closed.
7. Through a separate narrow PR, atomically replace only active `release_graphs.v0.5.0-00` with the exact future graph and #499 campaign declaration and remove the candidate manifest; require exact merged-`main` and live IssueOps readback.
...
Any unreviewed-head, partial, raced, stale, or mismatched body, PR, implementation, hosted, or manifest-promotion state fails closed. The ignored drafts and candidate manifest remain the repair sources until exact `main`, authoritative map, and hosted readback agree; no issue becomes ready and no implementation handoff proceeds from the temporary pre-merge body/link gap or any partial migration state.

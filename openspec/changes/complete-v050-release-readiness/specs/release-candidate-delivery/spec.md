...
- **WHEN** trusted release-version classification selects stable, #499 has exact `stable_ready` evidence and is closed so every child is closed, task 26.7's final implementation-versus-diagram review and every child task and #492 task other than 26.6 are checked, the accepted RC plus repeated semantic, published-main, source/workspace, security, release-policy, and package-input preflight binds the exact stable input, and #492 is the sole open milestone issue with 26.6 the sole unchecked task

...
- Make pull-request CI lean without reducing proof: #497 uses one closed impact contract and one Python-standard-library planner for human and Dependabot pull requests, fails unknown/shared/planner-owning inputs to full proof, and keeps default-branch, scheduled, candidate, and release proof complete. #498 adds one exact-author Dependabot campaign/audit automation contract, while #499 owns the sole v0.5.0 campaign issue and final release-window dispositions without per-PR issues or a second planner/store.

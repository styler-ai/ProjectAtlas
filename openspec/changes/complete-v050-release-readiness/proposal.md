...
- Prevalidate every native relationship request against one exact declared missing-or-extra transition before mutation; repair closed reverse dependents when blockers reopen; treat demilestoning as targeted graph drift; and dynamically fail published-snapshot proof on wrong roots, malformed identities, Git failures, or default-branch movement while ignoring only untracked notes excluded from tracked state.

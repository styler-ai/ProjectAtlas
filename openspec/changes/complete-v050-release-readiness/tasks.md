...
- [ ] 26.6 After an accepted release candidate, require #499's post-RC pre-stable audit and full-window final dispositions to produce exact `stable_ready` with every finally accepted campaign PR authenticated as merged at its exact head/merge commit and included in accepted current `main` plus the exact stable input, close #499 so every child is closed, repeat the fresh full-set Sol semantic reconciliation and objective published-main IssueOps readback, complete and check task 26.7's final implementation-versus-diagram review, and only then satisfy the stable prepublication finalization gate with #492 the sole open issue and 26.6 the sole unchecked task; with explicit promotion authorization repeat the complete installed and hosted proof for stable v0.5.0, verify v0.5.0 becomes Latest and downstream pins agree, synchronize OpenSpec/issues/reviews and check 26.6, close #492 last and close the milestone, then pass the postpublication final gate by rereading the exact hosted stable state with every mapped issue closed and every mapped task checked.

...
- [ ] 26.1 Freeze one exact v0.5.0 RC candidate revision, the native hierarchy with #492 as parent of every other milestone issue, the declared direct-blocker graph, the complete supported CLI/MCP/host/public-route inventory, accepted compatibility dispositions, platform matrix, isolated-fixture policy, and holistic success criteria; require every implementation-bearing child other than release-governance campaign #499 and every required review to be closed successfully, require #499 to be exact `candidate_ready` for that revision while #499/#492 remain open, and resolve every accepted issue's mapped OpenSpec task source plus architecture URL, heading, and Mermaid from exact published `main` before RC acceptance.

...
- [ ] 3.2 Update the version-matched ProjectAtlas plugin guidance and its generated support references to route PHP tasks through overview, folders, files, summary, outline, search, detailed graph evidence, and exact slice; require matching manifests and skill assets in both the plugin source and installed cache before installer readiness or refresh success without adding a generic language-guidance framework.

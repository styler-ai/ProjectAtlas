...
- [ ] 1.4 Map ordered issue-owned task slices, freeze the graph-derived prerelease-versus-stable milestone gate and credential-safe hosted-audit/finding-disposition contracts, prepare the exact sanitized acceptance-oriented thirty-body migration, and keep candidate task ownership mapped while staging the twenty-nine-child future graph and #499 campaign declaration in one non-authoritative manifest; after independent Sol and new hosted Codex review accept the corrected exact planning-PR head, require primary Sol to publish/read the thirty bodies while the PR remains open and the authoritative/live graph stays at twenty-five children, keep the temporary body-to-main architecture-link gap fail-closed for readiness, rerun normal unfiltered IssueOps/CI to green, authorize/merge/read exact planning `main`, then require accepted Luna objective checker/forms/guidance implementation/readback before primary Sol applies/reads the complete hosted milestone/native bootstrap and a separate narrow PR atomically promotes only the exact future graph/campaign and removes the manifest; require exact merged-main, live IssueOps, hosted, and eight-question semantic readback before readiness, and keep this independent shared task unchecked until #500 itself completes its final architecture review, then let primary Sol synchronize it outside #500's own checklist.

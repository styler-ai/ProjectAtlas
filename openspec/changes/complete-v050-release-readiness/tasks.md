...
- [ ] 1.4 Map ordered issue-owned task slices, prepare the exact sanitized issue-body migration with the restored acceptance-oriented section order, reconcile the twenty-nine-child native release hierarchy/dependency graph, preserve candidate-local OpenSpec/Mermaid validation for planning PRs, and require objective IssueOps plus the eight-question Sol semantic review from an exact clean live-default-branch readback before readiness, milestone assignment, native relationships, or implementation handoff; keep this independent shared task unchecked until #500 itself is complete after the migration, structural gate, semantic review, and published-main readback all succeed, then let primary Sol synchronize it outside #500's own checklist.

...
- [ ] 1.4 Map ordered issue-owned task slices, prepare the exact sanitized issue-body migration with the restored acceptance-oriented section order, keep candidate task ownership mapped while staging the twenty-nine-child future hierarchy/dependency graph and #499 campaign declaration in one non-authoritative promotion manifest so the active release graph stays exact to hosted state, and require controlled body publication, hosted milestone/native bootstrap, exact graph promotion, objective IssueOps, plus the eight-question Sol semantic review from an exact clean live-default-branch and hosted readback before readiness or implementation handoff; keep this independent shared task unchecked until #500 itself is complete after migration, structural gates, semantic review, graph promotion, and published-main readback all succeed, then let primary Sol synchronize it outside #500's own checklist.

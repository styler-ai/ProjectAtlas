...
- [x] 26.1 Freeze one exact v0.5.0 candidate revision, the native hierarchy with #492 as parent of every other milestone issue, the declared direct-blocker graph, the complete supported CLI/MCP/host/public-route inventory, accepted compatibility dispositions, platform matrix, isolated-fixture policy, and holistic success criteria; require every child issue and required review to be closed successfully and every accepted issue's mapped OpenSpec task source plus architecture URL, heading, and Mermaid to resolve from exact published `main` before release acceptance.
- [x] 26.2 Build the exact candidate with the Rust 1.98.0 workspace, database, IssueOps, OpenSpec, documentation, security/dependency, package, installer, asset, checksum/integrity, and release-policy gates; run the publication-admission gate from an exact clean checkout of the live default-branch revision, fail on candidate-only, stale, dirty, missing, or malformed issue evidence, abort on revision drift, reject partial accepted work, and preserve v0.4.5 as Latest.

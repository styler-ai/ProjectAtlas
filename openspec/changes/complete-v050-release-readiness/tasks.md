...
- [ ] 26.1 Freeze one exact v0.5.0 candidate revision, the native hierarchy with #492 as parent of every other milestone issue, the declared direct-blocker graph, the complete supported CLI/MCP/host/public-route inventory, accepted compatibility dispositions, platform matrix, isolated-fixture policy, and holistic success criteria; require every child issue and required review to be closed successfully before release acceptance.
- [ ] 26.2 Build the exact candidate with the Rust 1.98.0 workspace, database, IssueOps, OpenSpec, documentation, security/dependency, package, installer, asset, checksum/integrity, and release-policy gates; abort on revision drift, reject partial accepted work, and preserve v0.4.5 as Latest.
...
- [ ] 26.4 Run one clean holistic installed-product E2E spanning binary/npm/plugin/host installation, init, scan, purpose-led navigation, PHP/documents/graph/analysis, worktree/watcher/telemetry, parser capability, host configuration consumption, update/repair/uninstall, concurrency, cancellation, failure recovery, and compatible rollback without reading or mutating unrelated host or project state.

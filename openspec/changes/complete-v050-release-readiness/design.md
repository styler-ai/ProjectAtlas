...
A candidate checkout is the authority for proposed OpenSpec structure, issue mirrors, Markdown headings, locked-parser Mermaid syntax, rendered communication, semantic review, and the exact future graph encoded in one change-local promotion manifest. It is not publication proof for a durable `/blob/main/` issue link or live graph authority. `openspec/issue-map.json.changes` maps #497-#500 to their OpenSpec tasks because every immediate `tasks.md` requires an owner, but those mappings do not assign release membership or native relationships; authoritative `release_graphs.v0.5.0-00` continues to match the current hosted graph.

The corrected exact planning-PR head first receives independent Sol review and a new hosted Codex review. While that reviewed PR remains open, primary Sol publishes and reads back the thirty exact bodies from its reviewed drafts without changing the live twenty-five-child graph. Their temporary links to architecture not yet on `main` authorize no readiness; they exist only so normal unfiltered IssueOps/CI can validate the real bodies and turn the planning PR green. Primary Sol then authorizes the planning merge and reads the exact `main` artifacts back. Luna next lands, receives review for, and reads back the objective checker/self-test/forms/repository-guidance integration and any proven necessary version-matched plugin update, without owning GitHub/specification/personal-skill state. Only then does primary Sol apply and read back the full hosted milestone/native twenty-nine-child bootstrap. A separate narrow graph-promotion PR atomically replaces only active `release_graphs.v0.5.0-00` with the manifest's exact #499 campaign declaration and graph and removes the manifest.
...
Before wave 1, the planning PR publishes the shared OpenSpec, architecture sources, drafts, candidate task mappings, and non-authoritative future-graph manifest while the active release graph remains exact to current GitHub. The corrected exact head receives independent Sol and new hosted Codex review. Primary Sol then publishes/reads the thirty bodies while the reviewed PR remains open, preserving the active twenty-five-child graph and treating the temporary body-to-main architecture-link gap as non-readiness state. Normal unfiltered IssueOps/CI must turn green before Sol authorizes the planning merge and reads exact `main`. Luna then lands/reviews/reads the objective repository mechanism. Primary Sol next bootstraps/reads the complete hosted milestone/native graph, and a separate narrow atomic PR promotes only the future graph/campaign while removing the manifest. Exact merged-main, live IssueOps, hosted, and semantic readback must agree before any handoff; no Luna product worktree starts from candidate-only, partially bootstrapped, or structurally valid but semantically unreconciled evidence.

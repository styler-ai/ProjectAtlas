...
#477 pins `tree-sitter-php` 0.24.2 against Tree-sitter 0.26.13 and owns PHP 8 grammar registration plus conservative exact namespace/import/include/call relations. Dynamic PHP and mixed HTML gaps remain typed partial coverage. #339 follows #477 and publishes exactly one evidence-derived PHP guidance profile from the registry, generated support, fixtures, representative repositories, and installed skill.

#465 supports only PDF and DOCX. The fixed `pdf-extract` 0.12.0+projectatlas guest with `lopdf` 0.44.0, the `wasmi`/`wasmi_core` 2.0.0 host, `quick-xml` 0.42.0, and `zip` 0.6.6 (ZIP default features disabled, only `deflate` enabled) are pinned and their locked transitive trees are audited before adoption. DOCX admits only stored or DEFLATE `word/document.xml` and rejects encrypted or unsupported compression; PDF runs no script/external reference. Evidence locators are page/text span or part/paragraph/run/text span. Input, expansion, entries, recursion, time, memory, output, cancellation, coverage, and sparse-link policy are explicit; any necessary SQLite delta lands before adapters.

PDF parsing uses one fixed, embedded WASI guest interpreted by pinned `wasmi` 2.0.0 inside the existing symbols crate. Native `lopdf` callbacks cannot bound allocations or cancellation inside object loading and font/text parsing; the guest provides that missing containment boundary. The seven native workspace crates remain unchanged; `packaging/pdf-parser` is the separately built source of this one artifact, with its own audited lockfile and no runtime module-selection API. The retained canonical text formatter handles page and Form text; narrowly documented source patches preserve font resource scope, reject incomplete character codes and content streams, and skip image pixels. Every declared page-tree edge is validated, and non-image streams share an aggregate expansion ceiling before formatting. The host grants only bounded entropy and an empty environment; output and exit imports trap, and no filesystem, network, clock, or process capability is linked. Each document owns an isolated store capped at 64 MiB linear memory, a 1 MiB interpreter value stack, 256 call depth, and 500 million total fuel with cancellation checks between resumable slices. The resource limiter preserves resumable allocation-side fuel exhaustion while trapping actual memory denial. A local ten-second ceiling also respects any earlier caller deadline. Trusted fixed code is eagerly translated once, because lazy translation can fail outside resumable fuel execution. The document staging envelope is not a process RSS claim: shared translation and native allocator overhead require separate measured platform proof. Locked guest rebuild verification compares embedded bytes; ordinary native builds do not invoke nested Cargo or install a target.
...
#388 is deferred to v0.6.0 as a thin npm adapter with one package identity, exact tuple/asset/version/SHA-256 authority, explicit Node/npm floor, scripts-disabled route, proxy/offline semantics, process-safe cache locking, staged verification, and atomic activation. It never owns another runtime or database.
...
#491 installs one collision-safe `atlas` forwarder to the same runtime while preserving `projectatlas`. Ownership is bound to an independent installer-created provenance record containing the canonical forwarder/runtime pair plus a random capability held in the installer state directory; public forwarder text alone is never sufficient. Replacement files are staged and verified, published with platform-native no-clobber creation, and prior owned pairs are retired by same-directory quarantine, byte verification, and identity-safe deletion so a concurrent foreign replacement is preserved. `atlas health [report flags]` is the read-only report; `atlas health resolve ...` retains administration; `health-check` remains a compatibility alias.
...
3. #342 follows #488; #476 follows #481; #486 follows #482/#483.

...
- The all-closed milestone gate could make intentional RC governance state impossible or a broad exception could admit unfinished product work. Mitigation: select the prerelease path only from trusted version classification, derive exactly the release-root/campaign roles from the active graph, bind candidate-ready evidence to the release input, require every other issue closed/complete, and leave stable all-closed.

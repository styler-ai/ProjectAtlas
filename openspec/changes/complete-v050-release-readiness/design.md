...
The same #499 region records `collecting`, exact `candidate_ready`, or exact `stable_ready`. Candidate readiness requires the pre-RC audit and every candidate-snapshot record final for the exact candidate, permits RC1 while #499/#492 remain open, and is invalidated by an intervening pre-publication event. After accepted RC1, intake continues; stable readiness requires the later pre-stable audit and every new/full-window record final, after which #499 closes and stable #492 acceptance may begin.

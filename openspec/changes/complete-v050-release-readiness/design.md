...
A candidate checkout is the authority for proposed OpenSpec structure, issue mirrors, Markdown headings, locked-parser Mermaid syntax, rendered communication, semantic review, and the exact future graph encoded in one change-local promotion manifest. It is not publication proof for a durable `/blob/main/` issue link or live graph authority. Sol first lands those artifacts through a planning PR without a native closing issue. `openspec/issue-map.json.changes` maps #497-#500 to their OpenSpec tasks because every immediate `tasks.md` requires an owner, but those mappings do not assign release membership or native relationships; authoritative `release_graphs.v0.5.0-00` continues to match the current hosted graph. After exact-main planning readback, Sol publishes the accepted bodies, bootstraps and rereads hosted milestone/native state, then replaces that active release graph with the manifest's exact #499 campaign declaration and twenty-nine-child graph through a narrow publication; the manifest is removed in that promotion. The snapshot admits only the addressed Git top-level root with a well-formed local HEAD equal to the well-formed live default-branch ref SHA and no tracked changes; ignored untracked notes remain outside tracked publication identity. Root mismatch, malformed local or remote identity, Git timeout/process/OS failure, default-branch movement, or any body/hosted/map mismatch fails closed. Only the fully promoted stable snapshot may authorize `status:ready`, implementation handoff, merge, or release. Planned-issue, implementation-reference, merge-authorization, milestone, and release paths fail closed on candidate-only, stale, dirty, missing, or malformed evidence. This preserves the bootstrap without weakening the live graph reconciler or allowing a candidate worktree to impersonate published state.
...
- Candidate-local specifications or graph nodes could be mistaken for durable issue evidence. Mitigation: map task ownership without implying readiness, publish one non-authoritative future-graph promotion manifest, keep the active release graph equal to hosted state, then bind readiness and release gates to exact body/hosted bootstrap, graph promotion, clean live-default-branch readback, and every mapped task source and architecture target.
...
Before wave 1, the planning PR publishes the shared OpenSpec, architecture sources, drafts, candidate task mappings, and non-authoritative future-graph manifest while the active release graph remains exact to current GitHub. Sol then publishes and reads the bodies, performs the controlled hosted milestone/native bootstrap, promotes the manifest's exact graph and campaign declaration into the active release graph, and requires exact live-default-branch plus hosted readback and semantic reconciliation of every accepted issue explanation, acceptance section, link, diagram meaning, and task mirror. No Luna implementation worktree starts from candidate-only, partially bootstrapped, or structurally valid but semantically unreconciled evidence.

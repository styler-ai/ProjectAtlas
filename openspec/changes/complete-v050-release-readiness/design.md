...
#342 measures the existing reverse-caller query/service path and changes it only for a material winner with exact alias/fairness/plan compatibility. #358 evaluates existing worker pools under one process budget and accepts only an exact-graph resource win. The runtime creates one bounded pool per non-empty operation, reuses it for parser work, graph identity admission, and structural summaries, and merges parallel graph reports in source order; publication remains synchronous and atomic. The CLI scan route and MCP/watch routes all feed the same `SymbolBuildOptions` budget. #456 accepts a released-main baseline only for measured net benefit, exact revision/digest/schema identity, private writable copy, and safe full-init fallback.

...
- Re-run the complete publication checker immediately before both release creation and asset upload, using the existing shared pre-mutation helper. Install its locked Mermaid parser in the publish job. A successful verification snapshot must not admit publication after an owner closes or a child reopens during packaging.

...
- Mutating another process's environment or terminating any pre-existing ProjectAtlas, Codex, terminal, or unrelated process. The installer may terminate only the exact runtime-info probe process tree it started after that probe exceeds its time or output bound.

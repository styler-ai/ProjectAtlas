...
- Require a final `accepted`, `deferred`, `declined`, or `superseded` disposition for every PR record; `pending` and `provisional` block the applicable stage. An unlinked pre-PR finding may be deferred, declined, or superseded but never accepted. `accepted` requires exact hosted linkage to a real PR whose record is finally accepted; other linked findings remain blocking until their real PR record reaches another authorized final disposition. Provenance such as `retrospective_precontract` is recorded separately and never substitutes for a disposition.

...
- Require a final `accepted`, `deferred`, `declined`, or `superseded` disposition for every inventory record; `pending` and `provisional` block the applicable stage. Provenance such as `retrospective_precontract` is recorded separately and never substitutes for a disposition.
...
- Drive the bounded pre-RC and post-accepted-RC pre-stable audits solely from `.github/dependabot.yml` through one trusted default-branch `workflow_dispatch`, launchable with `gh workflow run`. Translate the supported hosted configuration into pinned official Dependabot CLI job input without dropping a field, bound per-entry/total time, output, and operation counts, and emit only `clean`, `findings`, or `failed` into the same campaign inventory.
...
- Record two exact states in the same campaign region: `candidate_ready` binds the pre-RC audit and final inventory snapshot to the exact RC candidate while #499 and #492 stay open, and `stable_ready` binds a later pre-stable audit plus every newly discovered record after the accepted RC, after which #499 closes and stable #492 acceptance may proceed.

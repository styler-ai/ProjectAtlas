...
- Drive the bounded pre-RC and post-accepted-RC pre-stable audits solely from `.github/dependabot.yml` through one trusted default-branch `workflow_dispatch`, launchable with `gh workflow run`. Translate the supported hosted configuration into pinned official Dependabot CLI job input without dropping a field, bound per-entry/total time, output, and operation counts, and emit only `clean`, `findings`, or `failed` into the same campaign region. Findings create or refresh pre-PR records using audit/update identity only; a real PR number, author, head, milestone, and campaign relationship are recorded only after exact hosted PR readback and linkage.

...
Allowed final dispositions are `accepted`, `deferred`, `declined`, and `superseded`. Intake begins `pending`; a not-yet-final judgment may be `provisional`. Both non-final states block the applicable readiness checkpoint. `Accepted` is the only merge-positive disposition: authenticated hosted readback must show `MERGED`, bind the reviewed source revision and resulting merge identity, and prove that merge is included by ancestry in accepted current `main`. A green or merge-authorized but still-open PR cannot be finally accepted. `Deferred`, `declined`, and `superseded` remain authorized non-merge outcomes for the exact recorded head/state and rationale. Provenance is orthogonal: `retrospective_precontract` identifies a PR merged before the campaign contract without becoming a fifth disposition or claiming post-hoc pre-merge authorization. A final disposition records a short repository-technical rationale and, where applicable, the superseding or corrective PR.
...
Merge authorization remains explicit and one-shot. It reads back exact author, campaign mapping, PR number/head/body/milestone, inventory record, review decision, unresolved threads, every protected check, current base, and branch synchronization. Only an explicit Sol dispatch bound to the reviewed dependency change, its source revision, and behavior-relevant diff may authorize the existing `issueops-merge-authorized` context. There is no automatic merge or gate waiver. Any new commit changes that binding, invalidates the authorization, and requires fresh proof/readback/dispatch. Authorization is still pre-merge state: only later authenticated readback of `MERGED`, the reviewed source revision and resulting merge identity, and accepted-current-`main` ancestry/inclusion may finalize `accepted`; an open/green PR or a merge not included in current `main` remains pending or provisional.
...
- **A green or authorized open PR could be marked accepted and omitted from the release input** → separate pre-merge authorization from post-merge acceptance, bind authenticated merged state, reviewed source and resulting merge identity, and current-main inclusion, and require every accepted merge in the exact checkpoint input.

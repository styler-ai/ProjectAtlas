...
The parity target SHALL be a reviewed, version-controlled accepted capability-set manifest with an immutable identity and digest. The initial accepted manifest SHALL contain the explicit 215-mode union, preserve all 63 current public modes, and map those modes to the currently ratified 209 normalized parser capabilities. The manifest SHALL distinguish the 11 standard-name aliases from runnable modes, keep CFML script/tag and ObjectScript UDL/routine/export-XML dialects as separate runnable modes, and disclose every justified parser and bounded-transform reuse. The ObjectScript export-XML mode SHALL reuse the ObjectScript UDL parser only after its own bounded transform contract passes. Each required entry SHALL identify its stable ProjectAtlas capability ID, runnable mode, canonical parser capability, aliases or reuse, required support tier, parser/pack provenance, fixture inventory, owner, and required release platforms. All accepted rows SHALL remain pending and unadvertised until their required evidence passes. Generated counts MAY summarize this set, but the numeric floor, aliases, extra capabilities, or higher aggregate scores SHALL NOT compensate for one missing or failing required entry.
...
### Requirement: Bounded Pre-Parse Transforms
An accepted mode that transforms a container before grammar parsing SHALL own a typed deterministic transform contract, distinct detection route, bounded input/output/record/depth limits, and source mapping from every derived fact and diagnostic back to the original file. XML-based transforms SHALL disable DTDs, entity expansion, external resources, schema fetches, and execution. Empty, malformed, oversized, nested-adversarial, and multi-record containers SHALL fail or report partial coverage without guessed symbols. A transform failure SHALL never fall through to an unrelated parser or promote the mode above its achieved tier.

#### Scenario: ObjectScript export contains multiple classes
- **WHEN** a bounded valid export container holds multiple ObjectScript UDL class records
- **THEN** ProjectAtlas deterministically transforms and parses each record through the pinned UDL parser while preserving original-file source identity and per-record provenance

#### Scenario: Export container requests an external entity
- **WHEN** an XML export declares a DTD, external entity, schema fetch, or another external resource
- **THEN** ProjectAtlas rejects the transform before parsing, performs no external access or execution, preserves the active index, and reports the mode as partial or unavailable

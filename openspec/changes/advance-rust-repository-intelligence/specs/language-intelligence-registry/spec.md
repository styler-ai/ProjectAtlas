...
The parity target SHALL be a reviewed, version-controlled accepted capability-set manifest with an immutable identity and digest. The initial accepted manifest SHALL contain the explicit 212-mode union, preserve all 63 current public modes, and map those modes to the currently ratified 207 normalized parser capabilities. The manifest SHALL distinguish the 11 standard-name aliases from runnable modes, keep CFML script and tag dialects as separate modes, and disclose every justified parser reuse. Each required entry SHALL identify its stable ProjectAtlas capability ID, runnable mode, canonical parser capability, aliases or reuse, required support tier, parser/pack provenance, fixture inventory, owner, and required release platforms. All accepted rows SHALL remain pending and unadvertised until their required evidence passes. Generated counts MAY summarize this set, but the numeric floor, aliases, extra capabilities, or higher aggregate scores SHALL NOT compensate for one missing or failing required entry.

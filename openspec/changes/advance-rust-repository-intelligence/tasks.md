...
- [ ] 3.1 Make every normal index-backed read, watch, and one-shot refresh detect relevant local edits, deletes, renames, ignore/configuration changes, and optional VCS-state changes after edits or restart; reconcile a safe bounded delta or return typed `refresh_required` instead of serving known-stale file, summary, symbol, relation, or search results in git or non-git source trees.
  - Tests: `projection_refresh_cannot_replace_publication_contract` (owning database transition), `normal_reads_do_not_serve_offline_stale_index_state` (shared real CLI/MCP Git and non-Git behavior, including publication-contract laundering), `mcp_project_path_overrides_keep_projects_isolated` (per-call project/root isolation after source changes), and `skipped_symbol_builds_invalidate_stale_symbols` (skipped projection invalidation). This shared coverage grows with task 3.1; it is not a one-test-per-task receipt.

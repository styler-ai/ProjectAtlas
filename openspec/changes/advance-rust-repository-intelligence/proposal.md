...
- Introduce a generated, version-locked language registry with explicit detection, parsing, symbol, semantic-resolution, and benchmarked-support tiers. The initial accepted set SHALL enumerate the explicit 212-mode union, preserve all 63 current public modes, and map the union to the currently ratified 207 normalized parser capabilities after standard-name aliases and justified parser reuse are disclosed. These are pending delivery targets, not achieved support claims. Parity means every accepted ProjectAtlas capability ID passes its declared tier; raw counts alone never compensate for a missing or failing entry.

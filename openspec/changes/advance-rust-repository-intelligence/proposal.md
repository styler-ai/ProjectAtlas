...
The program is ready for phased implementation after Phase 0 freezes its benchmark contracts, dependency decisions, decision-enabling local feasibility labs, and explicit release blockers. Full paired-corpus campaigns, calibrated release measurements, and hosted package/platform proof run after the relevant implementation exists and stabilizes; Phase 0 does not require evidence for a product that has not yet been built. Repository-intelligence architecture, refactoring, and functionality execute before the separate repository-wide test-saturation program. Every stable behavior still lands with focused unit tests and any risk-required integration, CLI/MCP E2E, or packaged smoke evidence; only the legacy/refactored-code saturation pass, final adjusted-coverage closure, complete source-mutation campaign, and release evidence wait until the functional architecture has stabilized. Later phases MUST remain gated on measured correctness, integrity, memory, latency, packaging, and cross-platform results rather than feature-count claims.

## What Changes

- Introduce a generated, version-locked language registry with explicit detection, parsing, symbol, semantic-resolution, and benchmarked-support tiers. The initial accepted set SHALL enumerate the explicit 215-mode union, preserve all 63 current public modes, and map the union to the currently ratified 209 normalized parser capabilities after standard-name aliases, ObjectScript dialect modes, and justified parser/transform reuse are disclosed. These are pending delivery targets, not achieved support claims. Parity means every accepted ProjectAtlas capability ID passes its declared tier; raw counts alone never compensate for a missing or failing entry.
...
- Do not defer focused tests for new stable behavior to the final coverage campaign; defer only repository-wide saturation and final release-quality evidence that would otherwise be invalidated by planned refactoring.
...
- **Testing:** focused unit, integration, CLI/MCP E2E, and affected-platform smoke tests land with each stable behavior. After the architecture and feature set stabilize, fixtures expand into the repository-wide legacy/refactored saturation, final adjusted-coverage, complete source-mutation, corruption, Unicode-path, crash-recovery, incremental-equivalence, clean-host plugin-store installation, and pinned-baseline comparison campaigns with explicit timeouts.

...
One closed `CoverageEnforcement` enum distinguishes feature-phase `implementation_checkpoint` evidence from final `release_quality` evidence. Both modes validate the pinned LLVM schema/tool, selected root and source confinement, per-file and aggregate count reconciliation, nonempty applicable counts, exact exceptions, and established platform floors. Only `release_quality` enforces the final raw and adjusted percentages. Omission defaults to `release_quality`, the validator summary and retained result bind the mode, and a release aggregate rejects checkpoint evidence. This keeps intermediate hosted platform proof honest without allowing it to masquerade as post-stabilization quality closure.

No pull request may lower a floor, reduce applicable source scope, silently increase exclusions, or relabel uncovered source as non-applicable. A floor may rise only to a value demonstrated by retained passing evidence for the same platform and policy scope. The agreed hard v0.4 targets are 98% raw/100% adjusted lines, 95% raw/98% adjusted regions, 98% raw/100% adjusted functions, and 90% raw/95% adjusted viable mutation kills. An unmet target remains visible with a tracking issue and keeps final `release_quality` CI, review, and release evidence blocked; an exception may define an honest adjusted applicability denominator but cannot waive the agreed numeric target.

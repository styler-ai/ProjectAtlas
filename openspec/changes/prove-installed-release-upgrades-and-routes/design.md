...
Prove complete installed CLI route execution and real v0.4.5-to-candidate continuity on all four supported native tuples. Reuse the current runner, installers, artifact servers, snapshots, and assertions. Repair the demonstrated Unix legacy-root upgrade refusal through the existing explicit root-transition boundary. Do not add unrelated runtime behavior, a new schema, external dependencies, an alternative test framework, npm delivery, or automatic stable promotion.
...
- Document repair with the checksum-verified candidate's `root set <path> --transition adopt-legacy`, followed by an installer retry. The packaged test first proves invalid-candidate refusal preserves the predecessor, then uses this explicit repair where required and completes candidate installation. A causal SQLite failure test proves adoption rollback and successful retry; explicit adoption recovers active worktree native columns from the existing filesystem crate's bounded Git inventory and stored administrative lifecycle fingerprints, never from lossy predecessor paths. Missing, ambiguous, or changed native evidence refuses the transaction. Generic migration and retired historical identity semantics remain unchanged; existing collision checks still run.
- Verify the selected location and opened database again after commit before reporting adoption success. SQLite locks do not prevent external Unix directory-entry replacement. A change detected before commit rolls back; a change detected after commit returns a distinct committed-location error without claiming rollback or attempting destructive file restoration. The caller stops before configuration publication or installer retry and preserves both locations for inspection.

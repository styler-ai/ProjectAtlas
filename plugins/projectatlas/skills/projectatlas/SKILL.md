...
5. Confirm MCP registration uses the generated project-local config whenever possible:
   ...
   The installers make their own active process prefer the verified runtime, repair the Windows LocalAppData stable mirror for bare `projectatlas` PATH use, remove verified stale ProjectAtlas shims only from known user-local Cargo/npm-style locations, and report unknown PATH shadows with an actionable warning. MCP configs and Codex registry entries stay pinned to the verified runtime path instead of the mutable stable mirror. When `codex` is available, the installers also inspect `codex mcp get projectatlas` and automatically replace a stale global Codex MCP registry entry with the verified runtime, current project DB, current config, and matching `--require-version`; set `PROJECTATLAS_SKIP_CODEX_MCP_REGISTRY_UPDATE=1` only when a managed environment intentionally owns that global registry. After plugin/runtime updates, agents must verify `codex mcp get projectatlas` or `codex mcp list` points to the verified runtime and version, and rerun the installer if it does not. A parent Codex, Claude Code, or OpenCode process may still need a restart before bare `projectatlas` lookup sees a newly added PATH entry, so generated absolute MCP configs remain the supported registration path.
...
- `token` reports estimated ProjectAtlas token savings. The default report is a fast offline `chars/bytes / 4` workflow heuristic, not provider/model billing-token accounting. Token output includes measured/gross/deduped/headline totals plus bucket, baseline, confidence, accounting layer, provider/model/backend, and accuracy labels.

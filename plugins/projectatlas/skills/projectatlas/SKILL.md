...
- `stale` is a legacy-compatible purpose status, not an automatic result of a meaningful source change; current migrations normalize legacy stale approvals and normal refreshes keep accepted purposes approved.
...
- File purposes are curated selectively. Fill them when they affect current work, public API, build/config/workflow/test/runtime behavior, routes, migrations, commands, MCP surfaces, or trusted metadata whose recorded responsibility is inconsistent. Do not review every file in a large repository unless the user explicitly asks for broad file-purpose cleanup.
...
- If the agent notices a wrong, vague, generic, inconsistent, or genuinely repurposed accepted purpose while working, correct it explicitly with `atlas_purpose_set` or `projectatlas purpose set` after inspecting enough context. Purpose quality should improve during normal work, but source changes alone never trigger correction.
...
When init, session brief, or the purpose queue returns an actionable `low`-scope handoff, keep the main task moving and, when the host supports isolated bounded subagents, delegate that exact batch to a purpose curator at the lowest reasoning tier the host can enforce. If the host cannot enforce subagent execution or reasoning selection, the main agent may process the same bounded batch. Give the curator only the queue rows plus bounded current summary, graph, outline, or exact-slice context; do not ask it to browse the whole repo. Copy each row's `task`, `work_key`, and `state_token` into `atlas_purpose_review` or `projectatlas purpose review --apply` so a stale curator cannot overwrite an accepted or changed row. Keep successful maintenance out of normal conversation and return only a minimal machine-facing result when the host requires one. ProjectAtlas exposes the handoff; the Rust server does not spawn an agent.

The actionable queue contains only missing or suggested purposes. It skips accepted purposes unless an agent or user separately assigns a known inconsistency or correction; use `atlas_purpose_set` or `projectatlas purpose set` for that deliberate correction path. Source/hash/summary/symbol/graph changes never demote or invalidate accepted purposes; absent paths leave purposes dormant and renames do not transfer approval. Never edit SQLite directly, and never expand an automatic handoff to `medium` or `strict` scope.
...
7. If init returns an actionable purpose handoff, let the host delegate that exact `low`-scope batch to an isolated purpose curator at its lowest enforceable reasoning tier while the main task continues. If the host cannot do that, process the same bounded batch in the main agent. Apply with the returned state tokens through ProjectAtlas purpose APIs only.
...
| --- | --- | --- | --- |
| Startup state and ranked candidates | `atlas_session_brief`, then `atlas_overview` only when broader repository state is still needed | Selected project, freshness, blockers, ranked folders/files, and a ready-to-use typed recommendation | Call the returned `atlas_file_summary`, `atlas_search`, `atlas_symbol_relations`, or `atlas_slice` request directly; do not rerun folder/file ranking already performed by the brief, and refresh first only when the response says the index is stale |
...
| Inspect architecture, impact, dead-code, or a static path from a known anchor | `atlas_symbol_relations` with the analysis view and closed `architecture`, `impact`, or `trace` mode | Candidate-labeled components, communities, cycles, purpose alignment/drift, structural complexity/bottlenecks, VCS-aware impact/dead-code, or a node-simple static path with coverage, selectors, totals, work, and explicit truncation | Follow the returned exact relation or source selector with `atlas_symbol_relations`, `atlas_file_summary`, or `atlas_slice`; treat `candidate` and `inconclusive` as review evidence rather than architectural truth |
...
| Read exact source evidence | `atlas_slice` with a narrow line/symbol range and optional `output_bytes` | Verbatim line or exact-symbol source under the exact adapter-output ceiling | Stop when the evidence answers the task; narrow the selector before raising the 256 KiB default, and otherwise follow the selected file's summary/relation route rather than broad-reading adjacent files |
...
7. `atlas_symbols` for declaration lookup and `atlas_symbol_relations` for detailed inbound/outbound traversal or the closed architecture, impact, and static-trace analysis views.
...
13. `atlas_session_brief` for compact startup orientation: selected project identity, index state, ranked candidates, blockers, and next-call recommendations without scanning or telemetry writes.
14. `atlas_settings` and `atlas_watch_status` for diagnostics. `atlas_settings` includes additive `mcp_session` capabilities for nearest-project policy, path scope, telemetry mode, scan policy, runtime identity, and no-secret guarantees.
15. `atlas_task_status` and `atlas_task_cancel` for the bounded MCP task-progress contract. Current scan/watch/search/summary/slice and CLI commands stay synchronous.
16. `atlas_reset_index` dry-run first when the local SQLite/cache state is corrupt or intentionally discarded.
17. `atlas_strip_legacy_purpose` only after migrated `.purpose` metadata is safely stored in SQLite.
18. `atlas_purpose_queue` when an agent needs a folder-first purpose curation queue before approving or correcting generated purposes.
19. `atlas_purpose_set` when an agent-approved purpose should be written to the durable index.
20. `atlas_purpose_review` when a reviewed batch should be previewed or applied to SQLite through the ProjectAtlas MCP surface.
21. `atlas_health_resolve` when a deterministic conflict is intentionally correct and should not be repeated.
22. `atlas_init`, `atlas_runtime_info`, `atlas_root`/`atlas_root_set`, `atlas_config`, `atlas_ignore_list`/`atlas_ignore_init_gitignore`/`atlas_ignore_add`/`atlas_ignore_remove`, `atlas_lint`, `atlas_mcp_config`, and `atlas_map` for CLI parity admin/reporting workflows when those tools are exposed.

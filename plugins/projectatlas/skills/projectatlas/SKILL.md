...
Use the returned typed recovery state rather than switching to paths: `ambiguous` returns bounded selectors; `init_required` returns the exact alias init call; `refresh_required` names the stale alias; invalid or mismatched Git evidence fails closed; `worktree_required` asks for an exact active checkout when a bare/common manager cannot select one. A manager with `core.worktree`, an enabled `config.worktree` override, or unresolved config includes never guesses its parent. If Git recreates a worktree administrative directory at a previously registered path, remove the stale ProjectAtlas alias and add the replacement explicitly; ProjectAtlas will not let the replacement inherit the alias or touch its atlas through that stale mapping. Lifecycle identity requires creation time plus platform file-object identity (Unix device/inode or Windows retained-handle volume/128-bit file ID); if the filesystem cannot provide the complete evidence, alias registration fails closed and explicit per-call `project_path` remains the recovery route. `project_path` remains the compatibility route for unregistered and older workflows, while alias routing is the normal concurrent-agent path.

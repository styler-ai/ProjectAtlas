...
fn show_text(gs: &mut GraphicsState, s: &[u8],
    ...
    let mut characters = font.char_codes(s);
    while !characters.i.as_slice().is_empty() {
        let (c, length) = characters.next().ok_or_else(|| std::io::Error::new(std::io::ErrorKind::InvalidData, "incomplete or unmapped PDF character code"))?;

...
Git can use non-UTF-8 path bytes on Unix, but the registration catalog and MCP JSON require lossless UTF-8 common-directory, administrative-directory, and source-root identities. Such a Git worktree remains visible only as a blocked structural row: it has no registrable selector or root, is never joined through lossy display text, and cannot be registered by alias. Use an exact filesystem-aware workflow outside alias registration rather than trying to reconstruct the replacement-character path.

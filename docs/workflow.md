...
Pull-request CI checks out the live `refs/pull/<number>/merge` ref for `pull_request`, `pull_request_review`, and `pull_request_review_comment` events, rather than trusting a stale payload SHA; non-PR events fall back to `github.sha`. The `pull_request` trigger retains `opened`, `synchronize`, and `reopened` and also reruns on `edited` so adding or removing a native closing reference cannot reuse a stale successful gate. It rejects PR-derived and push runs whose target branch is not the repository default branch; `dev` is retired and cannot be used to bypass the merge gates. For every PR-derived event, CI reads the live PR head and complete GitHub-native `closingIssuesReferences`, publishes `issueops-implementation` as `pending`, validates those references with the checked-out default-branch IssueOps checker, and publishes `success` or `failure` on that exact PR head. Foreign, missing, malformed, or conflicting identities fail closed. A pull request with no native closing issue is a planning slice and receives a successful implementation status after the ordinary checklist gate. Dependabot pull requests retain their dependency-update exemption for implementation references but still receive the status context and the standard CI quality gates; the default-branch delivery guard runs before that exemption so Dependabot cannot use a non-default base to bypass the rule.

Issue state, readiness labels, and milestone changes can invalidate an already-green implementation gate. The issue-event IssueOps workflow therefore locates affected open implementation PRs, including PRs closing a direct or transitive dependent of the changed issue, for every subscribed issue event and republishes the same `issueops-implementation` status on each affected PR's exact live head. It validates against the checked-out default-branch checker, so an old workflow run or stale workflow revision cannot certify current policy. Each status is set to `pending` before validation and then to `success` or `failure`; the status is the durable merge-readiness surface and does not rerun an old `01-CI` workflow. This covers the green → readiness-label removal → blocked, green → reopened blocker → blocked, and closed blocker → green transitions. A closed declared issue is checked against every direct blocker and is reopened immediately when any blocker remains open; the repaired open state is then used for status publication before IssueOps rejects the triggering transition, so a red workflow cannot leave invalid tracker state behind. Ordinary no-blocker child closures remain valid. GitHub exposes native dependency and sub-issue changes through the `issue_dependencies` and `sub_issues` webhooks, which are GitHub App events rather than Actions triggers, so those relationship changes use the workflow's manual issue revalidation input until an App delivery exists.

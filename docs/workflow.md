...
Pull-request CI reads GitHub's native `closingIssuesReferences` field and runs `--implementation-issue` for each closing issue. A pull request with no native closing issue is a planning slice and remains allowed to proceed with the ordinary checklist gate; text such as `Relates to #NNN` is not treated as a merge claim. Dependabot pull requests retain their dedicated dependency-update path and are intentionally exempt from the project issue-reference and implementation-blocker checks.

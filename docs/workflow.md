...
When `openspec/issue-map.json` declares a milestone `release_graphs` entry, the checker reconciles its direct `blocked_by` edges and derived release-root child set with GitHub's native issue dependencies and sub-issue/parent hierarchy (API version `2026-03-10`). `--planned-issue` validates shape and synchronization while blockers may remain open; `--implementation-issue` is the explicit implementation/merge-readiness check and fails until every direct blocker is closed. Missing or extra native edges, malformed graphs, unknown issues, self-edges, cycles, or an incorrect parent hierarchy fail closed.

Pull-request CI runs on GitHub's PR merge result for `pull_request`, `pull_request_review`, and `pull_request_review_comment` events. It rejects PR-derived and push runs whose target branch is not the repository default branch; `dev` is retired and cannot be used to bypass the merge gates. For an allowed PR, CI reads GitHub's native `closingIssuesReferences` field and runs `--implementation-issue` for each closing issue. A pull request with no native closing issue is a planning slice and remains allowed to proceed with the ordinary checklist gate; text such as `Relates to #NNN` is not treated as a merge claim. Dependabot pull requests retain their dedicated dependency-update path, but the default-branch delivery guard runs before that exemption so Dependabot cannot use a non-default base to bypass the rule.

...
Contract provenance is repository-controlled per issue in `openspec/issue-map.json` under `legacy_closed_issues`: every mapped issue not in that explicit set defaults to the new contract, while each set entry must remain mapped, closed, legacy-only, an exact mirror of its local owner slice, and complete with no unchecked historical task. A PR may introduce this set once when its accepted base predates the field; that initial boundary validates every derived/listed exception from the already fetched authenticated payloads before accepting it, and later PRs must preserve exact equality with the accepted base.

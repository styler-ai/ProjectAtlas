...
Pull-request CI checks out the live `refs/pull/<number>/merge` ref for `pull_request`, `pull_request_review`, and `pull_request_review_comment` events, rather than trusting a stale payload SHA; non-PR events fall back to `github.sha`. The `pull_request` trigger retains `opened`, `synchronize`, and `reopened` and also reruns on `edited` so adding or removing a native closing reference cannot reuse a stale successful gate. It rejects PR-derived and push runs whose target branch is not the repository default branch; `dev` is retired and cannot be used to bypass the merge gates. For an allowed PR, CI forwards each complete GitHub-native `closingIssuesReferences` object to IssueOps, which validates its repository identity before running the implementation blocker gate. Foreign, missing, malformed, or conflicting identities fail closed. A pull request with no native closing issue is a planning slice and remains allowed to proceed with the ordinary checklist gate; text such as `Relates to #NNN` is not treated as a merge claim. Dependabot pull requests retain their dedicated dependency-update path, but the default-branch delivery guard runs before that exemption so Dependabot cannot use a non-default base to bypass the rule.

Issue state, readiness labels, and milestone changes can invalidate an already-green implementation gate. The issue-event IssueOps workflow therefore locates affected open implementation PRs, including PRs closing a direct or transitive dependent of the changed issue, for every subscribed issue event and reruns their existing `01-CI / verify` workflow run against the exact current PR head; the existing `verify` context remains the required merge-readiness surface. If that run is still active, IssueOps cancels it, waits for a terminal state within a bounded window, and reruns it; a completion race is accepted only when a fresh status read proves the run completed, while authentication/network cancellation failures remain fatal. If no current-head run exists or cancellation never completes, the revalidation fails closed. This covers the green → readiness-label removal → blocked, green → reopened blocker → blocked, and closed blocker → green transitions. A closed declared issue is checked against every direct blocker and is reopened immediately when any blocker remains open, so a red workflow cannot leave invalid tracker state behind; ordinary no-blocker child closures remain valid. GitHub exposes native dependency and sub-issue changes through the `issue_dependencies` and `sub_issues` webhooks, which are GitHub App events rather than Actions triggers, so those relationship changes use the workflow's manual issue revalidation input until an App delivery exists.

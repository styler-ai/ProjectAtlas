...
Native dependency and sub-issue changes use the serialized `repository_dispatch` `issueops_relationship` route. Before any GitHub mutation or zero-mutation `already-satisfied` result, IssueOps derives the exact missing-or-extra transition from the selected declared release graph and current native state, then requires the requested kind, orientation, issue, related issue, and operation to match that transition exactly. A `blocked_by` source must belong to exactly one release graph; a `sub_issue` source must be exactly one graph's declared release issue. Unknown, unowned, multiply owned, ambiguous, unrelated-to-drift, or graph-widening requests fail before POST; an existing native relation does not admit removal or addition outside those ownership rules, and generic rollback is not a substitute for prevalidation. After an admitted idempotent mutation, IssueOps reads the relation back, reconciles the complete declared release graph, and emits exactly one correlated `applied`, `already-satisfied`, or `failed` terminal JSON outcome.

Issue events repair both directions of blocker drift within the declared graph. Closing an issue with an open direct blocker reopens it. Reopening a blocker derives a bounded reverse direct-blocker map, inspects its closed dependents, and reopens or fails every dependent whose blocker is open; each reopened dependent is processed through the same graph-bounded queue so transitive reverse dependents cannot remain falsely closed. For every affected pull request, IssueOps first validates that each closing reference names the addressed repository, then rereads the pull request's current exact head and complete closing references before each status or auto-merge operation. A foreign same-number issue, changed head, or changed closing-reference set is rejected or skipped for bounded retry and fails closed rather than mutating a stale target. Invalidation prioritizes revoking `issueops-merge-authorized` and disabling auto-merge where the existing safety route permits, then attempts failure for `issueops-implementation`; it continues across both contexts and every exact target while aggregating failures, so one failed status call cannot suppress later attempts. Complete API failure remains an explicit fail-closed/reporting outcome, not a claim that every remote status was revoked. The planned-issue job selects the issue's declared graph from the issue map even on a `demilestoned` event, so removal from the required milestone is targeted drift and cannot skip graph validation.

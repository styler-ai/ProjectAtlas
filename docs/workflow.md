...
Issue state and milestone changes can invalidate an already-green implementation gate. The issue-event IssueOps workflow therefore locates affected open implementation PRs, including PRs closing a direct or transitive dependent of the changed issue, and reruns their existing `01-CI / verify` workflow run against the exact current PR head; the existing `verify` context remains the required merge-readiness surface. If that run is still active, IssueOps cancels it, waits for a terminal state within a bounded window, and reruns it; if no current-head run exists or cancellation never completes, the revalidation fails closed. It handles supported `issues` events for edits, reopen, close, and milestone changes, including the green → reopened blocker → blocked and closed blocker → green transitions. A closed declared issue is checked against every direct blocker and is reopened immediately when any blocker remains open, so a red workflow cannot leave invalid tracker state behind; ordinary no-blocker child closures remain valid. GitHub exposes native dependency and sub-issue changes through the `issue_dependencies` and `sub_issues` webhooks, which are GitHub App events rather than Actions triggers, so those relationship changes use the workflow's manual issue revalidation input until an App delivery exists.
